...
def use_resource(resource, id, clock, sock, pending_requests, request_timestamp):
    print(f"|| <<P{id}>> entrou no recurso {resource} com clock [{clock[0]}]")
    time.sleep(3)
    print(f"|| <<P{id}>> liberou o recurso {resource} com clock [{clock[0]}]\n")
    for req in pending_requests[resource]:
        if req.src_process != id:
            rel = message(tipe=1, src_process=id, dst_process=req.src_process, resource=resource, timestamp=clock[0])
            sock.sendto(bytes(rel), process[req.src_process])
...
def deal_with_msg(msg, id, clock, ack_queue, sock, resource_state, pending_requests, request_timestamp):    
    ...
    if msg.tipe == 0 and msg.dst_process == id:
        ...
        if not resource_state[msg.resource] and (my_request_timestamp is None or msg.timestamp < my_request_timestamp or (msg.timestamp == my_request_timestamp and msg.src_process < id)): 
            ack = message(tipe=2, src_process=id, dst_process=msg.src_process, resource=msg.resource, timestamp=clock[0])
            ...
            nack = message(tipe=3, src_process=id, dst_process=msg.src_process, resource=msg.resource, timestamp=clock[0])
        ...
        print(f"<<P{id}>> || clock [{clock[0]}]: recebeu REL do <<P{msg.src_process}>> para recurso {msg.resource}\n")
        ack_queue[msg.resource].clear()
        pending_requests[msg.resource].clear()

        for pid, (host, port) in process.items():
            if pid != id:
                request_timestamp[msg.resource] = clock[0]
                pkt = message(tipe=0, timestamp=clock[0], resource=msg.resource, src_process=id, dst_process=pid)
...
def tr_send_msg(id, clock, resource_state, requisitions, request_timestamp):
    ...
    for i in range(requisitions):
        ...
        if not resource_state[required]:
            ...
            print(f"\t-> <<P{id}>> requisitando recurso {required} no clock <{clock[0]}> <-\n")
            for pid, (host, port) in process.items():
                if pid != id:
